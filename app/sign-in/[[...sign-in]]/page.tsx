  ...
  const handleEmailSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.status === 'complete') {
        ...
        // Handle partial sign-in states
        if (result.status === 'needs_second_factor') {
          setError('Two-factor authentication required. Please check your device.')
        } else if (result.status === 'needs_new_password') {
          setError('Please reset your password to continue.')
        } else {
          // Still try to activate and redirect
          try {
            await setActive({ session: result.createdSessionId })
            analytics.signIn('email')
            router.replace('/dashboard')
          } catch (activateErr) {
            setError('Sign-in successful but session activation failed. Please try refreshing the page.')
            setLoading(false)
          }
        }
      }
      ...
      if (errorMessage.includes('password') || errorMessage.includes('incorrect')) {
        setError('Email or password is incorrect. Please try again or use "Forgot password?" to reset.')
  ...
  const handleOAuthSignIn = async (provider: 'google' | 'apple' | 'facebook') => {
    ...
    try {
      ...
      if (isSafari && (errorMessage.includes('popup') || errorMessage.includes('blocked') || errorMessage.includes('redirect'))) {
        setError('Safari may have blocked the sign-in window. Please try using email/password sign-in, or allow pop-ups for this site in Safari settings.')
      } else {
        setError(errorMessage)
      }
    }
    ...
    <div className="min-h-screen flex items-center justify-center bg-white dark:bg-[#161616] px-4 transition-colors">
      <div className="relative w-full max-w-md rounded-none bg-white dark:bg-[#1a1a1a] border border-[#E3E3E3] dark:border-[#292929] p-10">
        ...
        <form className="space-y-4" onSubmit={handleEmailSignIn}>
            ...
            <div className="text-sm text-red-600 bg-red-50 dark:bg-red-900/20 dark:text-red-200 border border-red-200 dark:border-red-800 p-3 rounded-lg">
              ...
              {error.includes('Safari') && (
                <p className="text-xs mt-2 text-red-700 dark:text-red-300">
                  Tip: Email/password sign-in works reliably on all browsers, including Safari.
                </p>
              )}
            </div>
          )}
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <label 
                className="block text-sm font-medium text-[#161616] dark:text-white"
                style={{ fontFamily: 'var(--font-inter-tight), sans-serif' }}
              >
                Password
              </label>
              <a 
                ...
                className="text-xs text-[#505050] dark:text-[#ABABAB] hover:text-[#161616] dark:hover:text-white underline"
                style={{ fontFamily: 'var(--font-inter-tight), sans-serif' }}
              >
                Forgot password?
              </a>
        ...
        <p 
          ...
          style={{ fontFamily: 'var(--font-inter-tight), sans-serif' }}
        >
          Having trouble signing in? Email/password works on all browsers, including Safari.
        </p>
      </div>
    </div>
  )
